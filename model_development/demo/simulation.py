...
def disaridan_mr_tahmin_et(model, organ_ad, model_ad):
    print("\n" + "=" * 50)
    print(f"{organ_ad} / {model_ad}")
    print("=" * 50)
    
    # 🎯 Dinamik Terim Ayarı: Seçilen organa göre tıbbi terimleri belirliyoruz
    is_beyin = "beyin" in organ_ad.lower()
    goruntu_turu = "MR" if is_beyin else "CT"
    lezyon_adi = "Tumor" if is_beyin else "Tas"
    
    root = tk.Tk()
    root.withdraw() 
    root.attributes('-topmost', True) 
    
    print(f"\n  [*] Lutfen acilan pencereden analiz edilecek {goruntu_turu} goruntusunu secin...")
    
    dosya_yolu = filedialog.askopenfilename(
        title=f"{goruntu_turu} Goruntusu Sec",
        filetypes=[
    ...
    try:
        ...
        print(f"  [*] {goruntu_turu} goruntusu sisteme yukleniyor ve isleniyor...")
        ...
        print(f"  [*] Yapay zeka {lezyon_adi.lower()} aramasi yapiyor...")

        tahmin = model.predict(img_batch, verbose=0)
        t_maske = tahmin[0] 
        
        tahmin_analiz = tumor_analizi_yap(t_maske) # Altyapıdaki fonksiyon adın sabit kalıyor
        lezyon_var_mi = np.max(t_maske) > 0.5
        ...
        if lezyon_var_mi:
            ...
            print(f"\n Model herhangi bir {lezyon_adi.lower()} bulamadi.")
        ...
        plt.title(f"Yuklenen Ham {goruntu_turu} Goruntusu")
        plt.axis('off')
