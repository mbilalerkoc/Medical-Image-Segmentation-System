...
import DashboardLayout from '../../layouts/DashboardLayout'; // YENİ: Layout klasör yolu güncellendi
import styles from './Dashboard.module.css';

const Dashboard = () => {
  const [klinikAdi, setKlinikAdi] = useState('Klinik Yükleniyor...');
  const [doktorBilgisi, setDoktorBilgisi] = useState('Doktor Yükleniyor...');
  
  const [recentAnalyses] = useState([ /* ... statik verilerin aynen kalabilir ... */ ]);
  ...
  useEffect(() => {
    ...
    if (userStr) {
      ...
      const ad = userObj.ad ? userObj.ad : '-';
      const soyad = userObj.soyad ? userObj.soyad : '-';

      setDoktorBilgisi(`${unvan} ${ad} ${soyad}`);
    }
  }, []);

  return (
    <DashboardLayout title={` ${doktorBilgisi} | ${klinikAdi}`} >
