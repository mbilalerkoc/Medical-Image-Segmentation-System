...
const NewAnalysis = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    // DÜZELTME: Doktor ID'sini localStorage'dan güvenli bir şekilde alıyoruz
    const userStr = localStorage.getItem('user');
    const doktorId = userStr ? JSON.parse(userStr).id : null;

    // Eğer doktorId yoksa işlemi durdur, veritabanı hata verir
    if (!doktorId) {
      setError("Oturum hatası: Geçerli doktor bilgisi bulunamadı. Lütfen tekrar giriş yapın.");
      setLoading(false);
      return;
    }
    ...
    formData.append('doktor_id', doktorId); // DÜZELTME: Eksik olan bu satır eklendi
