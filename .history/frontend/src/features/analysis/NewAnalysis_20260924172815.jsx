...
// Yeni oluşturduğumuz alt bileşenleri çağırıyoruz
import AnalysisForm from './components/AnalysisForm';
import AnalysisResults from './components/AnalysisResults';
import OverlayModal from './components/OverlayModal';
import ReportModal from './components/ReportModal';

const NewAnalysis = () => {const navigate = useNavigate();
  ...
  useEffect(() => {
    ...
    img.src = `data:image/png;base64,${result.maske_base64}`;
  }, [result]);

  useEffect(() => {
    if (!preview || !processedMask) {
    ...
    baseImage.onload = () => {
      ...
      maskImage.src = processedMask;
    };
    baseImage.src = preview;
  ...
  const handleSubmit = async (e) => {
    ...
    // Gerçek oturum açmış kullanıcının bilgilerini alıyoruz
    const userStr = localStorage.getItem('user');
    const currentUser = userStr ? JSON.parse(userStr) : null;

    if (!currentUser || !currentUser.id) {
      setError("Oturum hatası: Geçerli doktor bilgisi bulunamadı.");
      setLoading(false);
      return;
    }
    ...
    formData.append('doktor_id', currentUser.id); // Veritabanının yabancı anahtar (FK) olarak beklediği gerçek ID
    formData.append('organ', organ);
    formData.append('sira_no', '1');
    formData.append('file', file);

    try {
      // Merkezi servisimizi çağırıyoruz
      const data = await predictAnalysis(model, formData);
      if (data.basarili) {
        setResult(data.sonuclar); 
        setAnalizId(data.analiz_id); 
      }
    } catch (err) {
  ...
  return (
    <DashboardLayout title="Yeni Analiz Başlat">
      <div className={styles.container}>
        ...
        <AnalysisForm 
          tcKimlik={tcKimlik} setTcKimlik={setTcKimlik}
          organ={organ} setOrgan={setOrgan}
          model={model} setModel={setModel}
          preview={preview} loading={loading} error={error}
          processFile={processFile} handleSubmit={handleSubmit}
        />

        <AnalysisResults 
          result={result} preview={preview} loading={loading}
          processedMask={processedMask}
          onOpenOverlay={() => setIsOverlayModalOpen(true)}
      ...
      <ReportModal 
        ...
        tcKimlik={tcKimlik} organ={organ} preview={preview} 
        result={result} processedMask={processedMask} 
        overlayImage={overlayImage} analizId={analizId} 
      />
    </DashboardLayout>
  );
};
