...
const ReportModal = ({
  ...
  const handleSaveAndShare = async () => {
    ...
    try {
      ...
      if (data.basarili) {
        alert(
          "Rapor başarıyla veritabanına kaydedildi ve hastanın paneline gönderildi!",
        );
        onClose();
      }
    } catch (err) {
      ...
      <div className="bg-white rounded-2xl w-full max-w-5xl max-h-[95vh] overflow-y-auto shadow-2xl flex flex-col print:shadow-none print:w-full print:max-w-none print:h-auto">
        ...
        <div className="p-6 bg-slate-50 border-t border-slate-200 flex justify-end gap-4 rounded-b-2xl print:hidden">
          ...
          <button
            ...
            disabled={isSaving}
            className={`flex items-center gap-2 px-6 py-2.5 text-white font-bold rounded-xl transition shadow-md ${isSaving ? "bg-blue-400" : "bg-blue-600 hover:bg-blue-700 shadow-blue-600/20"}`}
          >
