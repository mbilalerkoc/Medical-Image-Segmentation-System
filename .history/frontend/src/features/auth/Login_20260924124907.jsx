...
import { loginUser } from '../../services/authService'; // YENİ: Servis katmanını içeri aldık
...
const Login = () => {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      // YENİ: Axios post işlemi yerine doğrudan servisi çağırıyoruz. Kod çok daha temiz.
      const data = await loginUser(email, sifre);

      if (data.basarili) {
        localStorage.setItem('token', data.token);
        localStorage.setItem('user', JSON.stringify(data.kullanici));

        const role = data.kullanici.rol;
