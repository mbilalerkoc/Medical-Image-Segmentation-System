...
const NewAnalysis = () => {
  ...
  const [klinikId, setKlinikId] = useState('1'); // Şimdilik varsayılan 1
  ...
  // Form Gönderme (Postman'deki Send butonunun karşılığı)
  const handleSubmit = async (e) => {
    ...
    // 1. Form Data Oluşturma (Postman Body -> form-data)
    const formData = new FormData();
    formData.append('hasta_id', hastaId);
    formData.append('klinik_id', klinikId);
    formData.append('organ', organ);
    formData.append('sira_no', '1');
    formData.append('file', file); // Multer 'file' bekliyor

    try {
      // DİKKAT: Şimdilik Postman'den aldığın geçerli Token'ı buraya yapıştır.
      // İleride bunu localStorage'dan (giriş yapıldığında) çekeceğiz.
      const TEST_TOKEN = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpZCI6MSwicm9sIjoic3VwZXJhZG1pbiIsImlhdCI6MTc4OTkyODc2OSwiZXhwIjoxNzkyNTIwNzY5fQ.uhd1445VhYzpr-PuKGfR8NrhUjkNQwRs2bAbCtJv4ukr"; 

      // 2. Axios ile POST isteği atma
      ...
      // 3. Başarılı Sonucu State'e Kaydet
        ...
        <div className="w-full lg:w-1/3 bg-white p-6 rounded-xl shadow-md border-t-4 border-blue-600">
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <button 
              type="submit" 
              disabled={loading}
              className={`w-full py-2 px-4 rounded-md text-white font-medium shadow-md transition ${loading ? 'bg-slate-400' : 'bg-blue-600 hover:bg-blue-700'}`}
            >
        ...
        <div className="w-full lg:w-2/3 bg-white p-6 rounded-xl shadow-md border-t-4 border-green-500 flex flex-col items-center justify-center min-h-[400px]">
          ...
          {result && (
            <div className="w-full flex flex-col md:flex-row gap-6 items-center">
              ...
              {/* Orijinal ve Maske (Python'dan dönen base64) */}
              <div className="w-full md:w-1/2 flex flex-col items-center gap-4">
                 ...
                 {result.mask_base64 && (
                   <div className="relative">
                     {/* Base64 formatındaki maskeyi ekrana basıyoruz */}
                     <img src={`data:image/png;base64,${result.mask_base64}`} alt="Maske" className="max-h-[250px] rounded-lg shadow-sm border border-slate-200" />
                     <span className="absolute top-2 left-2 bg-blue-600 text-white text-xs px-2 py-1 rounded">AI Maskesi</span>
                   </div>
                 )}
              </div>

              {/* Metrikler ve Skorlar */}
              <div className="w-full md:w-1/2 space-y-4">
                ...
                <div className="flex justify-between items-center bg-slate-50 p-3 rounded-lg border border-slate-100">
                  ...
                  <span className="text-xl font-bold text-red-600">%{result.tumor_percentage?.toFixed(2) || 0}</span>
                </div>
                
                <div className="flex justify-between items-center bg-slate-50 p-3 rounded-lg border border-slate-100">
                  <span className="text-slate-600 font-medium">Dice Skoru</span>
                  <span className="text-lg font-bold text-blue-700">{result.dice_score?.toFixed(4) || '-'}</span>
                </div>

                <div className="flex justify-between items-center bg-slate-50 p-3 rounded-lg border border-slate-100">
                  <span className="text-slate-600 font-medium">IoU Skoru</span>
                  <span className="text-lg font-bold text-blue-700">{result.iou_score?.toFixed(4) || '-'}</span>
                </div>
