...
const NewAnalysis = () => {
  ...
  const handlePrintPDF = () => {
    // Geçici olarak tarayıcının yazdırma/PDF kaydetme özelliğini kullanıyoruz.
    // İleride buraya html2pdf.js kütüphanesini ekleyip tamamen arka planda PDF ürettirebiliriz.
    window.print();
  };
  ...
  const handleSaveAndShare = async () => {
    ...
    try {
      // 1. Önce PDF'i üret ve indir/gönder
      const element = reportRef.current;
      const opt = {
        margin:       10,
        filename:     `PathoVision_Rapor_${tcKimlik}.pdf`,
      ...
      // 2. Ardından backend reports tablosuna kaydı at
      const TEST_TOKEN = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpZCI6MSwicm9sIjoic3VwZXJhZG1pbiIsImlhdCI6MTc4OTkzMDgxNiwiZXhwIjoxNzkyNTIyODE2fQ.QVJEgEOsxfKx2jm0_0wbzlCVZhb0jExPQpGg_L_BVGQ"; 
      ...
      const payload = {
        ...
        analiz_id: analizId,
        doktor_id: doktorId,
        pdf_yolu: `reports/PathoVision_Rapor_${tcKimlik}.pdf`
      };
      ...
      if (response.data.basarili) {
        alert("PDF oluşturuldu ve rapor hastanın paneline / veritabanına bağlandı!");
        setIsReportModalOpen(false); 
      }
    } catch (err) {
      console.error("İşlem hatası:", err);
      alert("PDF/Rapor kaydı sırasında bir hata oluştu.");
    }
  };
