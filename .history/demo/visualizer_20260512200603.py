...
def disaridan_mr_tahmin_et(model, organ_ad, model_ad):
    ...
    dosya_yolu = input("\n  [*] Analiz edilecek MR goruntusunun tam yolunu girin\n  (Ornek: C:/resimler/hasta_mr.png): ").strip()
    
    # Tırnak işaretleriyle girilirse temizle
    dosya_yolu = dosya_yolu.replace('"', '').replace("'", "")
    
    if not os.path.exists(dosya_yolu):
        print(f"  [❌] Hata: '{dosya_yolu}' bulunamadi. Yolu kontrol edip tekrar deneyin.")
        return
