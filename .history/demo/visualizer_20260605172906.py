...
def bolum_tahminler(model, X_test, y_test, organ_ad, model_ad):
    ...
    # 🎯 DİNAMİK İSİMLENDİRME: Böbrek seçilirse CT, Beyin seçilirse MR yazar
    is_beyin = "beyin" in organ_ad.lower()
    goruntu_turu = "MR" if is_beyin else "CT"
    ...
    while True:
        ...
        for i, idx in enumerate(ornek_idxler):
            ...
            # 🎯 YENİ: Çevre/Sınır hatasını piksel cinsinden hesaplıyoruz
            sinir_hatasi = cevre_hata_hesapla(y_test[idx], tahminler[i])

            # 1. Sütun: Orijinal MR / CT
        ...
        print("\n  --- HESAPLANAN KLINIK VERILERIN KARSILASTIRMASI ---")
        for i, idx in enumerate(ornek_idxler):
            g = tumor_analizi_yap(y_test[idx])
            t = tumor_analizi_yap(tahminler[i])
            fark_mm2 = abs(g['alan'] - t['alan'])
            
            # 🎯 YENİ: Terminal tablosu için hesaplama
            sinir_hatasi = cevre_hata_hesapla(y_test[idx], tahminler[i])
            
            # 🎯 YENİ: Terminal çıktısının sonuna SINIR SAPMASI eklendi
            print(
                f"  {goruntu_turu} [{idx}] -> GERCEK: {g['alan']:.1f} mm² | TAHMIN: {t['alan']:.1f} mm² | FARK: {fark_mm2:.1f} mm² | SINIR SAPMASI: {sinir_hatasi:.1f} px")
