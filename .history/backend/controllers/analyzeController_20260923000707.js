...
const User = require('../models/User'); // TC'den hastayı bulmak için eklendi
...
exports.predictAndSave = async (req, res) => {
    try {
        ...
        // Frontend'den artık hasta_id değil, tc_kimlik geliyor
        ...
        // 2. TC Kimlik ile Hastayı Veritabanından Bulma
        // (Sadece 'hasta' rolüne sahip kullanıcıları arıyoruz ki doktorun TC'si ile karışmasın)
        const hasta = await User.findOne({ 
            where: { 
                tc_kimlik: tc_kimlik,
                rol: 'hasta'
            } 
        ...
        if (!hasta) {
            return res.status(404).json({ 
                basarili: false, 
                mesaj: `Sistemde ${tc_kimlik} TC numarasına sahip bir hasta bulunamadı.` 
            });
        }

        // Bulunan hastanın gerçek ID'sini alıyoruz (PostgreSQL ve Python kayıtları için kullanılacak)
        ...
        // Artık veritabanından bulduğumuz gercek_hasta_id'yi kullanıyoruz
        const analysis = await Analysis.create({
            hasta_id: gercek_hasta_id,
