...
exports.predictAndSave = async (req, res) => {
    try {
        ...
        // Multer'ın ayarına göre (DiskStorage veya MemoryStorage) veriyi güvenle Python'a aktarıyoruz
        if (file.buffer) {
            // Eğer dosya RAM'de (MemoryStorage) tutuluyorsa
            formData.append('dosya', file.buffer, {
                filename: 'upload_image.jpg',
                contentType: file.mimetype || 'image/jpeg'
            });
        } else if (file.path) {
            // Eğer dosya diske (DiskStorage) kaydediliyorsa
