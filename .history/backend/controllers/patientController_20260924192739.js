...
exports.addPatient = async (req, res) => {
  ...
  try {
    ...
    return res.status(201).json({
      ...
      mesaj:
        "Hasta başarıyla eklendi ve sisteme bağlandı.",
...
exports.getPatientById = async (req, res) => {
  try {
    ...
    if (
      Number(patient.doktor_id) !==
      Number(doctor.id)
    ) {
      return res.status(403).json({
        basarili: false,
        mesaj:
          "Bu hastayı görüntüleme yetkiniz bulunmuyor.",
      });
    }
    ...
    return res.status(200).json({
      ...
      hasta: {
        ...
        telefon:
          patient.User?.telefon || "",

        dogum_tarihi:
          patient.User?.dogum_tarihi ||
          null,
