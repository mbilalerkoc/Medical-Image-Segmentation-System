...
def tahmin_yap(model_adi: str, dosya_bytes: bytes) -> dict:
    ...
    # 6. Maskeyi listeye çevir (JSON'a dönüştürmek için)
    maske_liste = maske.tolist()

    return {
        "model_adi"        : model_adi,
        "maske"            : maske_liste,        # 256×256 binary dizi
        "metrikler": {
            "toplam_piksel"    : toplam_piksel,
            "patoloji_piksel"  : patoloji_piksel,
            "saglikli_piksel"  : saglikli_piksel,
            "patoloji_yuzdesi" : patoloji_yuzdesi,  # görüntünün yüzde kaçı patoloji
            "max_olasilik"     : max_olasilik,       # modelin en yüksek güveni
            "ort_olasilik"     : ort_olasilik,       # ortalama aktivasyon
        }
    }